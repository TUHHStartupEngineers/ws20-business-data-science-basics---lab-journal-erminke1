...
knitr::kable(top_10_us$organization, col.names = 'Company name', caption = 'Top 10 US companies by patent count')
...
# some tests to see if we are getting good data.



# santiy_check <- dt1[!duplicated(dt1$patent_id), ] # We see that some patents repeat.
# 
# sanity_check2 <- pat_assignee_tbl %>%
#   group_by(patent_id) %>%
#   summarize (
#     count = n()
#   )
...
corporations_id_names <- assignee_tbl %>%
  select(id, type, organization) %>%
  filter(type == 2) %>%
  rename(assignee_id = id)# This here makes sure they're from USA 
...
top_10_us <- right_join(top_patent_wrld, corporations_id_names, by = 'assignee_id') %>%
  head(10) %>%
  select(assignee_id)
...
uspc_tbl_filtered <- uspc_tbl %>%
  ...
  merge(top_10_us, by = 'assignee_id') %>%
